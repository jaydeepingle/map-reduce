...
int bitmap_find_first_bit(unsigned char * bitmap, int size, int val)
{
    ...
    for(i = 0; i < (size * BIT_PER_BYTE); i++) {
        tempValue = *(bitmap + i / BIT_PER_BYTE);
        sam = ((tempValue >> i % BIT_PER_BYTE) & 0x01);
        if(sam == val) {
            return i;
        }
    }
...
{
    int slot_number = (int)(target_pos / BIT_PER_BYTE);
    ...
    *(bitmap + slot_number) = *(bitmap + slot_number) | (0x01 << (target_pos % BIT_PER_BYTE));
...
{
    int i = 0, slot_number = (int)(target_pos / BIT_PER_BYTE);
    unsigned char temp, tempValue = (~(0x0) << 1);
...
{
    int slot_number = (int)(pos / BIT_PER_BYTE);
    if (NULL == bitmap)
    {
        printf("ERROR: NULL bit map!\n");
        return BITMAP_OP_ERROR;
    }
    return (*(bitmap + slot_number) >> (pos % BIT_PER_BYTE)) & 0x01;
}
